...
contract DeployScript is Script {
    function run() external {
        ...
        PolicyEngine policyEngine = new PolicyEngine(registry, killSwitch);
        MockFXEscrow fxEscrow = new MockFXEscrow(deployer);

        SettlementRouter router =
            new SettlementRouter(deployer, registry, policyEngine, killSwitch, IFXEscrow(address(fxEscrow)));
